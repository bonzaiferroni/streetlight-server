...
class S3BlobClient(env: Environment): BlobClient {
    ...
    private val client = S3Client {
        this@S3Client.region = this@S3BlobClient.region
    ...
    override suspend fun put(bytes: ByteArray, filename: String, contentType: String): Url? {
        return try {
            client.putObject {
                this@putObject.bucket = this@S3BlobClient.bucket
                key = filename
    ...
    override suspend fun delete(url: Url): Boolean {
        ...
        return try {
            client.deleteObject {
                this@deleteObject.bucket = this@S3BlobClient.bucket
                key = filename
            }
            true
        } catch (e: S3Exception) {
...
private val console = globalConsole.getHandle(S3BlobClient::class)