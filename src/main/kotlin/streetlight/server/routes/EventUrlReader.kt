...
class EventUrlReader(
    ...
    suspend fun serve(request: ParseRequest): MultiEventParseResponse {
        val parse: MultiEventParse? = when (request) {
            is UrlParseRequest -> agent.readUrl(request.url, ParserText.multiEventInstructions)
            is HtmlParseRequest -> agent.readHtml(request.url, request.html, ParserText.multiEventInstructions)
